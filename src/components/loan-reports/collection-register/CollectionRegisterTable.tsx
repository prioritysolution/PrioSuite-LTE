...
interface CollectionRegisterTableProps {
  data: CollectionRegisterRow[] | null;
  loading?: boolean;
  emptyMessage?: string;
}
...
export function CollectionRegisterTable({
  data,
  loading,
  emptyMessage = "No records found.",
}: CollectionRegisterTableProps) {
          ...
          <table className="w-full text-left border-collapse text-sm min-w-[1100px]">
            <thead className="bg-primary">
              <tr>
                ...
                <th className="px-5 h-12 text-[13px] font-bold text-primary-foreground uppercase tracking-wider text-right">
                  Principal
                </th>
                <th className="px-5 h-12 text-[13px] font-bold text-primary-foreground uppercase tracking-wider text-right">
                  Interest
            ...
            <tbody>
              {!hasData ? (
                ...
                pageRows.map((row, index) => (
                  <tr
                    key={`${row.groupName}-${row.memberName}-${row.issueDate}-${safePageIndex}-${index}`}
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-right text-gray-700 font-semibold whitespace-nowrap">
