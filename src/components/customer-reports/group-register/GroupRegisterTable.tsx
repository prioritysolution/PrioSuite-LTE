...
interface GroupRegisterTableProps {
  data: GroupRegisterRow[] | null;
  loading?: boolean;
  emptyMessage?: string;
}
...
export function GroupRegisterTable({
  data,
  loading,
  emptyMessage = "No records found.",
}: GroupRegisterTableProps) {
          ...
          <table className="w-full text-left border-collapse text-sm min-w-[980px]">
            <thead className="bg-primary">
              <tr>
                ...
                <th className="px-5 h-12 text-[13px] font-bold text-primary-foreground uppercase tracking-wider text-center">
                  No of Member
                </th>
            ...
            <tbody>
              {!hasData ? (
                ...
                pageRows.map((row, index) => (
                  <tr
                    key={`${row.groupNo}-${safePageIndex}-${index}`}
...
export default GroupRegisterTable;
