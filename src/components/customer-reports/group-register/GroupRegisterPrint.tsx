...
import { GroupRegisterRow } from "@/container/customer-reports/group-register/GroupRegisterType";

interface GroupRegisterPrintProps {
  printRef: React.RefObject<HTMLDivElement | null>;
  mappedList: GroupRegisterRow[];
    ...
    >
      ...
      <table className="w-full border-collapse text-center text-xs">
        <thead>
          ...
          <tr style={{ border: "none" }}>
            <td colSpan={7} style={{ border: "none", height: "16px" }}></td>
          </tr>

          <tr className="bg-slate-100 font-bold border-b border-black">
            <th className="border border-black p-2 w-[40px]">Sl.</th>
            <th className="border border-black p-2 text-left">Group No</th>
            <th className="border border-black p-2 text-left">Group Name</th>
            <th className="border border-black p-2 text-left">Area</th>
            <th className="border border-black p-2">No of Member</th>
            <th className="border border-black p-2">Admission Date</th>
        ...
        <tbody>
          {mappedList.map((row) => (
            <tr key={row.sl} className="border-b border-black">
              ...
              <td className="border border-black p-2">
                {formatDateStr(row.admissionDate)}
              </td>
              <td className="border border-black p-2 text-left">
                {row.underCo || "-"}
              </td>
...
export default GroupRegisterPrint;
