...
import { FieldErrors, UseFormRegister, Control, FormProvider, UseFormReturn } from "react-hook-form";
import { IPurpose } from "@/container/setup/purpose-master/PurposeMasterReducer";
import { IPurposeFormInput } from "@/container/setup/purpose-master/Hooks";
import InputField from "@/common/formFields/InputField";

interface PurposeUIProps {
  state: {
    isModalOpen: boolean;
    editId: number | null;
    searchTerm: string;
  ...
  isLoading: boolean;
  filteredData: IPurpose[];
  register: UseFormRegister<IPurposeFormInput>;
  control: Control<IPurposeFormInput>;
  errors: FieldErrors<IPurposeFormInput>;
  submitPending: boolean;
  onSubmit: (e?: React.BaseSyntheticEvent) => Promise<void>;
  handleEdit: (item: IPurpose) => void;
  openModal: () => void;
  closeModal: () => void;
  setSearchTerm: (term: string) => void;
  form: UseFormReturn<IPurposeFormInput>;
  deleteTarget: IPurpose | null;
  deletePending: boolean;
  handleDelete: (item: IPurpose) => void;
  closeDeleteModal: () => void;
  confirmDelete: () => void;
}

export const PurposeUI: React.FC<PurposeUIProps> = ({
  state,
  isLoading,
  filteredData,
  control,
  ...
  const columns: ColumnDef<IPurpose>[] = [
    {
      id: "sl",
      header: "Sl",
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: "Loan Purpose",
      cell: ({ row }) => {
        const purposeName = row.original.Purp_Desc || "Unknown";
        return (
        ...
        <div className="pl-2 flex items-start gap-3 sm:gap-4 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-gray-500 mt-1.5 font-medium">
              Define and manage purposes for issuing loans
            </p>
            <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-sm">
              <span className="font-semibold text-gray-800">
                {filteredData.length}{" "}
                {filteredData.length === 1 ? "purpose" : "purposes"}
              </span>
              <span className="text-gray-300">•</span>
              <span className="text-gray-500">Loan purpose setup</span>
            </div>
          </div>
        </div>
      ...
      >
        <DialogContent className="sm:max-w-[520px] p-0 overflow-hidden border border-gray-100 shadow-2xl rounded-xl font-sans gap-0">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                <DialogTitle className="text-xl font-bold text-primary tracking-tight">
                  {state.editId ? "Update Purpose Details" : "Create New Purpose"}
                </DialogTitle>
                <DialogDescription className="text-sm text-gray-500 mt-1 font-medium">
                  {state.editId
                    ? "Modify the loan purpose name below."
                    : "Enter a new purpose for issuing loans."}
                </DialogDescription>
              </div>
            </div>
          </div>
          ...
          <FormProvider {...form}>
            <form
              onSubmit={onSubmit}
              className="p-5 sm:p-6 space-y-5 bg-white"
              autoComplete="off"
            >
              <InputField
                control={control}
              ...
              <div className="form-actions">
                ...
                >
                  ...
                  {state.editId ? "Update Purpose" : "Save Purpose"}
                </Button>
              </div>
            </form>
          </FormProvider>
          ...
          <div className="p-5 sm:p-6 form-sections bg-white">
            <p className="text-sm text-gray-700 font-medium leading-relaxed">
              ...
              <span className="font-bold text-gray-900">
                {deleteTarget?.Purp_Desc || "this purpose"}
              </span>
              ?
            </p>
        ...
        <div className="form-sections">
          <section className="form-section">
            <div className="form-section-title">
              ...
              <h3 className="font-bold text-primary text-base">Purpose List</h3>
            </div>

            <div className="flex items-start gap-2.5 text-primary/80">
              <Target size={18} className="mt-0.5 shrink-0" />
              <p className="text-sm font-medium text-gray-500 leading-relaxed">
                Maintain loan purposes used across new loan application
                workflows.
              </p>
            </div>
