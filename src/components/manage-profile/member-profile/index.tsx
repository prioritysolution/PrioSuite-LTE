...
interface MemberProfileUIProps {
  ...
  methods: UseFormReturn<IMemberFormInput>;
  ...
  onFetchMember: (memNo: string) => void;
  submitPending: boolean;
}

export const MemberProfileUI: React.FC<MemberProfileUIProps> = ({
  state,
  methods,
  onSubmit,
  handleNextClick,
      ...
      <div className="page-header-card">
        ...
        <div className="pl-2 flex-shrink-0">
          ...
          <p className="text-sm text-gray-500 mt-1 font-medium">
            Register and manage member details efficiently
          </p>
        </div>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto animate-in fade-in slide-in-from-right-4 duration-300">
              {state.flowMode === "update" ? (
                <div className="flex items-center gap-2 w-full md:w-auto">
                  <div className="relative flex-1 md:w-[260px]">
                    ...
                    <Input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" &&
                        state.searchId &&
                        onFetchMember(state.searchId)
                      }
                      className="pl-9 pr-9 h-12 bg-slate-50 border-gray-200 focus-visible:ring-primary focus-visible:border-primary transition-all font-medium"
                    />
                    {state.searchId ? (
                      <button
                        type="button"
                  ...
                  <Button
                    onClick={() => onFetchMember(state.searchId)}
                    disabled={!state.searchId || fetchPending}
                    className="h-12 px-6 bg-primary hover:bg-[#024786] text-white font-bold rounded-lg shadow-sm transition-all flex items-center gap-2 whitespace-nowrap"
                  >
                    {fetchPending ? (
        ...
        <div className="animate-in fade-in slide-in-from-top-4 duration-500 page-body-card">
          <FormProvider {...methods}>
            <form onSubmit={onSubmit} className="form-sections">
              <BasicDetails editMode={state.editMode} />
              <ContactAndKycDetails />
              <AdditionalDetails />
              <AdmissionDetails editMode={state.editMode} />
